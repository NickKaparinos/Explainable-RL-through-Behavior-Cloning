...
def evaluate_agent(agent, env, predict_fn, num_episodes: int = 100) -> list:
    """ Evaluate agent on RL environment"""
    reward_list = []
    episode_reward = 0
    i = 0
    obs = env.reset()
...
def encode_video(video_frames_list: list, video_dir: str, video_num: int, fps: float = 60.0, prefix: str = '') -> None:
    """ Encode video frames list to .mp4 video """
...
def sklearn_predict_action(svm, obs):
    action = int(svm.predict(obs.reshape(1, -1))[0])
    return action
...
def compare_agents_performance(agent1_mean_reward, agent2_mean_reward):
    """ Compare agent 1 and agent 2 performance"""
